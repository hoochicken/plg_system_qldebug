...
class JFormFieldQldebuggeneratetable extends JFormField
{
    ...
    protected function getInput()
    {
        ...
        $this->generateTable();
        if(true==$this->obj_helper->checkIfTableExists($this->storage)) return '<a href="'.$this->getUrl().'&qldebugdroptable=1">'.sprintf(JText::_('PLG_SYSTEM_QLDEBUG_MSG_DROPTABLE'),$this->storage).'</a>';
        if(true!=$this->obj_helper->checkIfTableExists($this->storage)) return '<a href="'.$this->getUrl().'&qldebuggeneratetable=1">'.sprintf(JText::_('PLG_SYSTEM_QLDEBUG_MSG_GENERATETABLE'),$this->storage).'</a>';
    }
